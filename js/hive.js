...
class Hive extends SmellableObject {
	constructor(canvas, position, settings, collisionObjs, hiveNumber){

		// Standard args
		if (typeof(hiveNumber)==='undefined')
			hiveNumber = 0;
	...
	createAnt(){
		...
		var parentHive = this.hiveNumber;
		...
		if (this.settings.getAntType() == AntType.SIMPLE)
			newAnt = new AntSimple(this.getCanvas(), antPos, rotation, this.settings, this.collisionObjs, parentHive);
		if (this.settings.getAntType() == AntType.CUSTOM)
			newAnt = new AntCustom(this.getCanvas(), antPos, rotation, this.settings, this.collisionObjs, parentHive);
