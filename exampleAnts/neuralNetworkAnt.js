...
if (typeof(this.neuralNetwork.network)==='undefined')
		return [ActionType.MOVE, DirectionType.FORWARD, rand(-45,45)];
	...
	for (var objs in objsCategories) {
		for (var id in objsCategories[objs]) {
			...
			if(objsCategories[objs][id].getObjectType() == ObjectType.ANT) {
				...
				hiveList[targetArea] += (objsCategories[objs][id].getParentID() != this.getParentID()) ? 1 : -1;
			}
		}
	}
...
var networkAnswer = createFeatureVector.bind(this)();
